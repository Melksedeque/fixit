  ...
  if (!isUser && effectiveTab === 'metrics') {
    ...
    const avgByTechDisplay = avgByTech
      .map((row) => {
        ...
        return {
          name: tech?.name || '—',
          minutes: Math.round(row._avg.executionTime || 0),
        }
      })
    ...
    <div className="space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <Badge
            variant="secondary"
            aria-label={`Total de chamados: ${totalCount}`}
          >
            {totalCount}
          </Badge>
        </div>
          ...
          items={[
            {
              label: 'Chamados',
              value: 'tickets',
              href: `/tickets?${new URLSearchParams({ ...params, tab: 'tickets' } as Record<string, string>).toString()}`,
            },
            ...(isUser
              ? []
              : [
                  {
                    label: 'Métricas',
                    value: 'metrics',
                    href: `/tickets?${new URLSearchParams({ ...params, tab: 'metrics' } as Record<string, string>).toString()}`,
                  },
                ]),
          ]}
        />
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            <CardTitle className="text-sm font-medium">
              Gráfico por Status (
              {metrics?.period === 'last30' ? 'Últimos 30 dias' : 'Total'})
            </CardTitle>
            <div className="inline-flex rounded-md border border-border bg-muted/40 p-1">
              <Button
                asChild
                variant={
                  (metrics?.period || 'all') === 'all' ? 'default' : 'ghost'
                }
                size="sm"
              >
                <Link
                  href={{
                    pathname: '/tickets',
                    query: { ...params, created: undefined },
                  }}
                >
                  Total
                </Link>
              ...
              <Button
                asChild
                variant={
                  (metrics?.period || 'all') === 'last30' ? 'default' : 'ghost'
                }
                size="sm"
              >
                <Link
          ...
          <CardContent className="space-y-2">
            {[
              ...
              const pct = Math.round(
                (count / (metrics?.maxCountPeriod || 1)) * 100
              )
              return (
      ...
      {effectiveTab === 'metrics' && !isUser ? (
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0">
            ...
            <CardDescription>
              Refine os resultados por status, prioridade e responsável
            </CardDescription>
          </CardHeader>
          <CardContent>
            <form
            ...
            >
              ...
              <div className="w-[180px] shrink-0">
                <Select
                  name="priority"
                  defaultValue={params.priority || undefined}
                >
              ...
              <div className="flex items-center gap-2 shrink-0">
                <Button asChild variant="ghost">
                  <Link href="/tickets" aria-label="Limpar filtros">
                    Limpar
                  </Link>
                </Button>
      ...
      {effectiveTab === 'metrics' && !isUser ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {metrics && metrics.avgResMin > 0
                  ? `${Math.floor(metrics.avgResMin / 60)}h ${metrics.avgResMin % 60}m`
                  : '—'}
              </div>
      ...
      {effectiveTab === 'metrics' && !isUser ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {metrics && metrics.slaAvgHours > 0
                  ? `${metrics.slaAvgHours}h`
                  : '—'}
              </div>
          ...
          <Card>
            ...
            <CardContent>
              ...
              <CardDescription>
                Concluídos/fechados que excederam o SLA
              </CardDescription>
            </CardContent>
          </Card>
        </div>
              ...
              items={[
                {
                  label: 'Lista',
                  value: 'list',
                  href: `/tickets?${new URLSearchParams({ ...params, view: 'list', tab: 'tickets' } as Record<string, string>).toString()}`,
                },
                {
                  label: 'Quadro',
                  value: 'kanban',
                  href: `/tickets?${new URLSearchParams({ ...params, view: 'kanban', tab: 'tickets' } as Record<string, string>).toString()}`,
                },
              ]}
            />
          )}
