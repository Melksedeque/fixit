...
export async function createUser(prevState: ActionState | null, formData: FormData): Promise<ActionState> {
  ...
  if (session?.user?.role !== "ADMIN") return { error: "Apenas administradores podem criar usuários." }
...
export async function updateUser(id: string, prevState: ActionState | null, formData: FormData): Promise<ActionState> {
  const session = await auth()
  if (session?.user?.role !== "ADMIN") return { error: "Apenas administradores podem editar usuários." }
  ...
  const { name, email, password, role, whatsapp } = validatedFields.data
  const existingUser = await prisma.user.findUnique({ where: { email } })
  if (existingUser && existingUser.id !== id) return { error: "Este e-mail já está em uso por outro usuário." }

  const dataToUpdate: Prisma.UserUpdateInput = { name, email, role, whatsapp }
  if (password && password.length >= 6) dataToUpdate.password = await bcrypt.hash(password, 10)

  const file = formData.get("avatar") as File | null
  const avatarUrl = await saveFile(file)
  const removeAvatar = formData.get("removeAvatar") === "true"
  if (avatarUrl) dataToUpdate.avatar = avatarUrl
  else if (removeAvatar) dataToUpdate.avatar = null
...
export async function deleteUser(id: string) {
  const session = await auth()
  if (session?.user?.role !== "ADMIN") return { error: "Apenas administradores podem excluir usuários." }
  if (session.user.id === id) return { error: "Você não pode excluir sua própria conta." }
