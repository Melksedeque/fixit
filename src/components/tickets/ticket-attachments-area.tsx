...
export function TicketAttachmentsArea({
  name = 'attachments',
}: TicketAttachmentsAreaProps) {
      ...
      fetch('/api/attachments', {
        ...
        .then(async (res) => {
          ...
          if (payload.url) {
            ...
            updateItem(id, { status: 'error' })
            console.error('[attachments] upload missing url', {
              name: file.name,
              id,
            })
          }
  ...
  const processFiles = useCallback(
    (acceptedFiles: File[]) => {
      console.log('[attachments] processing files', {
      ...
      const newItems: AttachmentItem[] = acceptedFiles.map((file) => {
        ...
        console.log('[attachments] created item', {
          id,
          name: file.name,
          size: file.size,
        })
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      console.log('[attachments] onDrop triggered', {
        count: acceptedFiles.length,
      })
      ...
      if (target) {
        ...
        if (target.url) {
          fetch('/api/attachments', {
            ...
            console.error('[attachments] failed to delete from server', {
              id,
              error: err,
            })
          })
    ...
    <div className="space-y-2">
      ...
      >
        ...
        <p className="mt-1 text-[11px] text-muted-foreground">
          ou clique para selecionar arquivos do seu computador. Máximo 5MB por
          arquivo.
        </p>
        {items.length > 0 && (
